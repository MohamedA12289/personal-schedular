    ...
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-7 gap-2 rounded-2xl bg-white p-4 shadow-sm shadow-slate-200/70">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((label) => (
          <div
            key={label}
            className="text-center text-xs font-semibold uppercase tracking-[0.16em] text-slate-500"
          >
            {label}
          </div>
        ))}
            ...
            {week.map((day) => {
              ...
              return (
                ...
                >
                  ...
                  {isToday && (
                    <span className="text-[10px] font-semibold text-teal-600">Today</span>
                  )}
                  {dayEvents.length > 0 && (
...
function DayView({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const minutes = minutesSinceStartOfDay(new Date());
      setNowPosition(
        ((minutes - HOUR_RANGE.start * 60) / ((HOUR_RANGE.end - HOUR_RANGE.start) * 60)) * 100,
      );
    }, 60_000);
    return () => clearInterval(interval);
  }, [isToday]);

  const dayEvents = events.filter((event) => isSameDay(new Date(event.start), selectedDate));
    ...
    <section className="space-y-4">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white p-4 shadow-sm shadow-slate-200/70">
        <div className="relative">
          {Array.from({ length: HOUR_RANGE.end - HOUR_RANGE.start + 1 }, (_, idx) => idx + HOUR_RANGE.start).map(
            (hour) => (
              <div
                key={hour}
              ...
              >
                <div className="w-12 pt-1 text-right text-xs font-semibold text-slate-400">
                  {hour}:00
                </div>
              ...
              >
                ...
                <div className="mt-2 flex items-center gap-2 text-xs text-slate-600">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      onUpdateReminder(event.id, Number(e.target.value || "10"))
                    }
                  >
          ...
          {isToday && nowPosition > 0 && nowPosition < 100 && (
            <div className="absolute left-0 right-0" style={{ top: `${nowPosition}%` }}>
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-teal-500 px-3 py-1 text-[11px] font-semibold text-white shadow">
                  Now
                </span>
              </div>
            </div>
    ...
    <section className="space-y-3">
      ...
      <div className="space-y-2">
        {sorted.map((event) => (
          <div
            key={event.id}
            className="flex flex-col gap-1 rounded-xl border border-gray-200 bg-white p-3 shadow-sm"
          >
        ...
        {sorted.length === 0 && (
          <p className="text-sm text-slate-500">
            No events yet. Try adding one from the chat bar below.
          </p>
        )}
      </div>
    </section>
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="space-y-4 rounded-2xl bg-white p-4 shadow-sm shadow-slate-200/70">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Works best when the app is open or installed as a PWA.
            </p>
          </div>
        ...
        <p className="text-xs text-slate-500">
          Notifications stay local to this device. Reminders fire when the app or installed PWA is
          active.
        </p>
      </div>
      ...
      <div className="space-y-3 rounded-2xl bg-white p-4 shadow-sm shadow-slate-200/70">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Replace current events with a saved JSON file.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <input
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    events.forEach((event) => {
      ...
      const reminderTime =
        new Date(event.start).getTime() - event.reminderMinutesBefore * 60 * 1000;
      const delay = reminderTime - now;
      if (delay <= 0 || delay > 1000 * 60 * 60 * 24 * 3) return;

      const timer = setTimeout(() => {
        new Notification(event.title, {
          body: `${formatDate(new Date(event.start), {
            weekday: "short",
            month: "short",
            day: "numeric",
          })} at ${formatTime(new Date(event.start))}`,
          icon: "/icon-192.svg",
        });
      }, delay);
  ...
  const handleNaturalSubmit = (value: string) => {
    ...
    if (!parsed) {
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "system",
          text: "I couldn't understand that. Try something like 'math test at 7 pm Thursday'.",
        },
      ]);
      return;
    }
    ...
    setMessages((prev) => [
      ...
      { id: crypto.randomUUID(), role: "user", text: value },
      {
        id: crypto.randomUUID(),
        role: "system",
        text: `Added: ${parsed.title} on ${formatDate(parsed.start, {
          weekday: "short",
          month: "short",
          day: "numeric",
        })} at ${formatTime(parsed.start)}.`,
      },
    ]);
  ...
  const handleExport = () => {
    ...
    link.href = url;
    link.download = `my-schedule-backup-${formatDate(new Date(), {
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
    })}.json`;
  ...
  const dataInfo = `Export creates a JSON backup you can move to another device, and Import replaces current data. Currently storing ${events.length} event(s) locally.`;
            ...
            <main className="space-y-6 p-4 sm:p-6" aria-live="polite">
              ...
              {activeTab === "day" && (
                <DayView
                  ...
                  onUpdateReminder={(id, reminderMinutesBefore) =>
                    updateEvent(id, { reminderMinutesBefore })
                  }
                />
              )}
